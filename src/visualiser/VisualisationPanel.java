...
public class VisualisationPanel extends JComponent {
	...
	public void paintState(Graphics2D g2, ASVState s) {
		if (s == null) {
			return;
		}
	...
	public void displayCSpace(boolean t){
		sampling = t;
		repaint();
	...
	public void paintComponent(Graphics graphics) {
		...
		if(sampling){
			List<ASVState> states = problemSetup.getSampleStates();
			for(ASVState temp:states){
		...
		if (!animating) {
			if (displayingSolution) {
				List<ASVState> path = problemSetup.getPath();
